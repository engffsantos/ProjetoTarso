...
pessoa_view = Blueprint('pessoa_view', __name__)

@pessoa_view.route('/pessoas')
def listar_pessoas():
...
@pessoa_view.route('/pessoas/nova', methods=['GET', 'POST'])
def nova_pessoa():
    if request.method == 'POST':
        # Code to create a new person in the database
        PessoaModel.criar_pessoa(request.form)
        return redirect(url_for('pessoa_view.listar_pessoas'))
    else:
        return render_template('pessoa_form.html')

@pessoa_view.route('/pessoas/editar/<int:id>', methods=['GET', 'POST'])
...
@pessoa_view.route('/pessoas/excluir/<int:id>')
